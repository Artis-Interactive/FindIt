...
namespace findit_backend.Handlers.ProductHandler
{
    public class ProductHandler : BaseHandler, IProductHandler
    {
        ...
        public string CreateNonPerishableProduct(FullNonPerishableProductModel product)
        {
            ...
            var query2 = @"INSERT INTO [dbo].[NonPerishableProducts] ([ProductID], [Stock])
        ...
        {
            string query = $"SELECT * FROM dbo.NonPerishableProducts WHERE ProductID = '{productId}'";
            DataTable tableResult = CreateQueryTable(query);
            int stock = Convert.ToInt32(tableResult.Rows[0]["Stock"]);
